...
class TextProcessor:
    ...
    @staticmethod
    def sigmoid_normalize(matrix):
        """
        Сигмоидальная нормализация матрицы.
        
        :param matrix: Исходная матрица (numpy array).
        :return: Нормализованная матрица с сигмоидальной функцией.
        """
    ...
    def preprocess_data(self, ngrams_len=(2, 34+3)):
        ...
        if self.vectorizer_path is None:
            ...
            if self.tfidf_templ is None:
                self.tfidf_templ = self.vectorizer.transform(self.templates)
        self.templates = [self.normalize_string(t) for t in self.templates]
        self.ngrams = [self.normalize_string(n) for n in self.ngrams]
    ...
    def compare_substrings_with_templates(self, metric):
        """
        Сравнение шаблонов с n-граммами на основе расстояния Левенштейна.
        
        :return: 
            - results: Список кортежей (номер шаблона, шаблон, минимальное расстояние, номер n-граммы, соответствующая n-грамма).
            - best_template_info: Информация о шаблоне с минимальным расстоянием и соответствующей n-грамме.
        """
        # Вычисление матрицы расстояний Левенштейна с использованием rapidfuzz
        if metric == 'lev':
            self.templates = [self.normalize_string(t) for t in self.templates]
            self.ngrams = [self.normalize_string(n) for n in self.ngrams]
            distances_matrix = process.cdist(
                self.templates,  # Массив шаблонов
                self.ngrams,     # Массив n-грамм
                scorer=fuzz.ratio,  # Используем метрику схожести (чем выше значение, тем лучше)
                score_cutoff=0    # Минимальный порог схожести
            )
            self.cos_matrix = distances_matrix / 100
        elif metric == 'cos':
            self.cos_matrix = cosine_similarity(self.tfidf_templ, self.tfidf_matrix)
            ...
            self.cos_matrix = self.sigmoid_normalize(self.cos_matrix) 
        elif metric == 'jaro_winkler':
            # Расстояние Джаро-Винклера
            distances_matrix = process.cdist(
                self.templates,  # Массив шаблонов
                self.ngrams,     # Массив n-грамм
                scorer=JaroWinkler.similarity,  # Используем метрику Джаро-Винклера
                score_cutoff=0   # Минимальный порог схожести
            )
            self.cos_matrix = distances_matrix
            
        else:
            raise ValueError('Недопустимая метрика.')
        # Преобразуем схожесть в расстояние (чем больше схожесть, тем меньше расстояние)
        # distances_matrix = 100 - distances_matrix
    
        # Векторизованное вычисление минимальных значений расстояния и индексов n-грамм
        # min_distances = np.max(distances_matrix, axis=1)  # Минимальные расстояния для каждого шаблона
        # best_ngram_indices = np.argmax(distances_matrix, axis=1)  # Индексы n-грамм с минимальными расстояниями

        # # Найти общий минимум среди всех минимальных расстояний
        # overall_min_distance = np.max(min_distances)  # Наименьшее значение расстояния
        # first_min_index = np.argmin(min_distances)  # Первый индекс с минимальным расстоянием

        # # Получаем информацию о лучшем шаблоне
        # best_template_index = first_min_index
        # best_ngram_index = best_ngram_indices[first_min_index]
        # print(first_min_index)
        # # Формирование информации о лучшем шаблоне
        # best_template_info = {
        #     "template": self.templates[best_template_index],
        #     "distance": overall_min_distance,
        #     "ngram": self.ngrams[best_ngram_index]
        # }
        ...
        #         i + 1,  # Номер шаблона
        #         self.templates[i],  # Шаблон
        #         min_distances[i],  # Минимальное расстояние
        #         best_ngram_indices[i] + 1,  # Номер n-граммы
        #         self.ngrams[best_ngram_indices[i]]  # Соответствующая n-грамма
        #     )
        #     for i in range(len(self.templates))
    ...
    def normalize_string(self, s):
        # Приводим к нижнему регистру, убираем лишние пробелы и символы
        return re.sub(r'\s+', ' ', s.strip().lower())
    
    def run(self, phrase, metric='cos', ngrams_len=(2, 34+3)):
        """
        ...
        self.compare_substrings_with_templates(metric)
        ...
        self.max_value = np.max(self.cos_matrix)
        ...
        self.templ_num = max_index[0] + 1
        ##############
        print("Confidence:", self.max_value)
...
processor = TextProcessor(templates=templates_, column_name='test')
# processor.run('Да, слушаю Вас, машинист локомотива на подходе к станции Бабынино.', metric='cos')
# processor.run('Я машинист поезда  <NUMBER>  Спиридонов на приближении к станции  Следую с неисправными устройствами безопасности', metric='lev')
# processor.run('Аллё, да, прибываете на пятый путь выходной ДСП 1 Панков.')
# processor.save_data_with_pickle('enbedding.pkl', templates=True)

phrases = ['Машинист поезда 7 на приближении к станции Бабынино.', 
           'Привет, я машинист локомотива номер 131 на приближении к станции Москва, как дела?.',
             'Да, аллё, машинист состава 35 на подходе к станции Калуга-1, что у вас случилось?',
               'Я машинист поезда  номер 9 Спиридонов на приближении к станции  двигаюсь с неисправными тормозными системами.', 
               'Да, здравствуйте, я машинист локомотива  64 Николаев на подходе к Москве  иду с неисправными системами.',
            'Пропускаетесь по 22-му боковому без остановки ДСП 1 Панков.',
            'Да, понял Вас, по 13-му иду фронтальному иду напроход, пульт первый Ларин.', 
            'Аллё, да, машинист локомотива номер 131 на подъезде к Питеру. Что у вас?.',
            'Слушаю тебя машинист локомотива 55 Владимиров, иду с отказавшими системами контроля давления.',
            'Дежурный по станции Бабынино, Никитин, слушаю Вас.']
...
for phrase in phrases:
    ...
    df_conf.append(processor.max_value)
    df_templ.append(processor.orig_templ)

df = pd.DataFrame({'phrase': phrases, 'confidence': df_conf, 'predict_template': df_templ})
